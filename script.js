...
function tellMe(joke) {
    // console.log('tell me:', joke);
        VoiceRSS.speech({
            key: 'f6d9ad1cad9a4ccf85b824d1e9a2f9e7',
            src: joke,
            hl: 'en-us',
            r: 0,
            c: 'mp3',
...
async function getJokes() {
    ...
    try {
        ...
        if (data.setup) { //if there is a setup (only happen when we have 2 part joke)
            joke = `${data.setup} ...${data.delivery}`; //`` used to easily add strings and variables together into one string..
            //single and 2 part joke is separated by setup and delivery..
        } else {
            joke = data.joke;
        }
        //Text-to-Speech
